...
const SwimmingPool: React.FC<SwimmingPoolProps> = ({ count }) => {
  ...
  const poolWidth = count * laneWidth + 4;
  const poolLength = 56;
  const poolCenterZ = -25;
  const poolDepth = 3.2;

  const stadiumWidth = Math.max(70, poolWidth + 24);
  ...
  return (
    <group>
      ...
      {Array.from({ length: count }).map((_, i) => {
        const laneX = (i - (count - 1) / 2) * laneWidth;
        return (
      ...
      {/* Finish Wall with FINA Electronic Touchpads */}
      <group position={[0, -poolDepth / 2, -53.5]}>
        ...
        {Array.from({ length: count }).map((_, i) => (
          <mesh key={`pad-${i}`} position={[(i - (count - 1) / 2) * laneWidth, 0.8, 0.35]}>
      ...
      {Array.from({ length: count + 1 }).map((_, i) => {
        const ropeX = (i - count / 2) * laneWidth;
      ...
      {Array.from({ length: count }).map((_, i) => {
        const blockX = (i - (count - 1) / 2) * laneWidth;
        return (
      ...
      {/* ================= 5. INDOOR AQUATIC ARENA STRUCTURE (실내 수영장) ================= */}
      {/* Tiled Pool Deck (Floor around the pool) */}
      ...
      {/* Arena Side Walls (Indoor Enclosure) */}
      ...
      {/* High Arched Roof Ceiling with Skylights */}
      ...
      {Array.from({ length: 9 }).map((_, i) => {
        ...
        return (
          <group key={`truss-${i}`} position={[0, roofHeight - 1, trussZ]}>
            ...
            {/* Hanging LED Floodlight Fixtures */}
            <mesh position={[-15, -0.6, 0]}>
      ...
      {/* Underwater Pool Lights for Crystal Clear Water Glow */}
